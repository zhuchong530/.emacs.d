...
#+begin_src emacs-lisp
  (use-package highlight-indent-guides
    ...
    :config (highlight-indent-guides-mode 'column)
    )
#+end_src
...
- =highlight-symbol= to toggle highlighting of the symbol at point throughout the current buffer.
- =highlight-symbol-mode= to keep the symbol at point highlighted.
- functions =highlight-symbol-next=, =highlight-symbol-prev=, =highlight-symbol-next-in-defun= and =highlight-symbol-prev-in-defun= allow for cycling through the locations of any symbol at point.
- =highlight-symbol-nav-mode= to enable key bindings (M-p and M-p) for navigation. When =highlight-symbol-on-navigation-p= is set, highlighting is triggered regardless of =highlight-symbol-idle-delay=.
