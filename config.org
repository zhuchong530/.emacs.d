...
#+begin_src emacs-lisp
  ...
  ;; https://www.gnu.org/software/emacs/manual/html_mono/use-package.html#Troubleshooting
  ;;  enbable when pass '--debug-init' command line flags, for getting more information
...
#+begin_src emacs-lisp
  ...
  (setenv "WORKON_HOME"
          (expand-file-name "pythonCode/"
                            (expand-file-name "Programme/" (getenv "HOME"))))
#+end_src
...
- 自动折行
  ...
  全局启用会严重影响 isearch、avy、move-text 等操作的体验。仅在文本模式下启用
...
#+begin_src emacs-lisp
  ...
  (set-fringe-mode 10)
  (setq default-directory (expand-file-name "Programme/" (getenv "HOME")))
...
#+begin_src emacs-lisp
  ;; Emacs minibuffer configuration for vertico
  (use-package emacs
    :custom
    ...
    ;; 显示层级，否则recursive-minibuffer容易困惑
    ...
    ;; Hide commands in M-x which do not work in the current mode. Vertico-Mode & Corfu
    ;; commands are hidden in normal buffers. This setting is useful beyond Vertico and Corfu
...
*show-paren-style* controls whether just the two parens, or also the text between them get highlighted. The valid options here are:

- *parenthesis* (show the matching paren)
- *expression* (highlight the entire expression enclosed by the parens)
- *mixed* (highlight the matching paren if it is visible in the window, the expression otherwise).
...
- 最大化
  ...
  ~(toggle-frame-maximized)~ 直接在顶层调用，可能在帧未完全初始化时执行。使用 ~default-frame-alist~
...
#+begin_src emacs-lisp
  (use-package doom-themes
    ...
    ;; Enable flashing mode-line on errors or Emacs bell rings
    (doom-themes-visual-bell-config)
    ;; Enable cusotm neotree theme(nerd-icons must be installed!)
...
#+begin_src emacs-lisp
  (cl-case system-type
    ...
    (gnu/linux
     (set-face-attribute 'default nil :family "Monaspace Krypton" :height 170 :weight 'normal))
    (t
     (set-face-attribute 'default nil :family "Monaspace Krypton" :height 180 :weight 'normal)))
...
#+begin_src emacs-lisp
  (use-package nerd-icons-dired
    ...
    :hook (dired-mode . nerd-icons-dired))
#+end_src
...
#+begin_src emacs-lisp
  ;; improve long line coding, don't need bidirectional text
  ;; https://emacs-china.org/t/topic/25811/9
  ;; Disable bidirectional text scanning for a modest performance boost
  (setq-default bidi-display-reordering nil)
  ;; 进一步优化长行处理(Emacs28+)
  (setq bidi-inhibit-bpa t                ; given up some bidirectional functionality for slightly faster re-display
...
#+begin_src emacs-lisp
  (use-package which-key
    ...
    (which-key-separator " → ")
    (which-key-show-prefix 'left)    ; 弹窗里显示当前前缀
    (which-key-show-early-on-C-h t)  ; 前缀后按 C-h 立即弹
...
#+begin_src emacs-lisp
  (use-package comment-dwim-2
    ...
    :custom
    (comment-dwim-2-move-to-comment-column t) ;;; 注释时对齐到注释列
    (comment-dwim-2-comment-style 'indent))      ;; 使用缩进风格注释
#+end_src
...
#+begin_src emacs-lisp
  ...
  (use-package rainbow-mode
    :if (display-graphic-p)
    :delight
    :hook ((css-mode html-mode web-mode sgml-mode latex-mode org-mode) . rainbow-mode))
#+end_src
...
从@minad’s orderless configuration 偷来的 https://github.com/minad/consult/wiki#minads-orderless-configuration
...
mac上4K分辨率会导致这个插件巨卡，稍微调低一点点就很流畅。现在分辨率是 3360x1890
...
#+begin_src emacs-lisp
  (use-package corfu
    ...
    :bind (:map corfu-map
                ...
                ("M-p" . corfu-previous)
                ("TAB" . corfu-insert))
    ;; Enable Corfu globally.(Recommended).
    ;; since many modes provide Capfs and Dabbrev can be used globally(M-/).
    :custom
...
#+begin_src emacs-lisp
  (use-package yasnippet
    ...
    :custom (yas-snippet-dirs (list "~/.emacs.d/snippets"))
    :config
    (yas-global-mode 1))
#+end_src
...
macos file descriptors limitation,https://www.reddit.com/r/emacs/comments/x4p7mg/fix_annoying_max_open_files_for_emacs/
...
#+begin_src emacs-lisp
  (use-package eglot
    ...
    (eglot-stay-out-of '())
    ;; drop log to impove performance
...
#+begin_src emacs-lisp
  (use-package eldoc-box
    :hook ((prog-mode eglot-managed-mode) . +eldoc-box-hover-at-point-mode-maybe)
    :init
    (defun +eldoc-box-hover-at-point-mode-maybe (&optional arg)
      (when (display-graphic-p)
        (eldoc-box-hover-at-point-mode (or arg 1)))))
#+end_src
...
需要GNU Emacs 27.1+。 Flycheck自己本身并不检查buffer，而是依赖外部程序来检查，具体见：[[https://www.flycheck.org/en/latest/languages.html#][list of supported language]]
...
#+begin_src emacs-lisp
  (use-package flycheck
    ...
    (setq flycheck-eglot-exclusive nil))  ;允许多checker
#+end_src
...
#+begin_src emacs-lisp
  (use-package flycheck-rust
    :hook ((rust-mode rust-ts-mode) . flycheck-rust-setup))
#+end_src
...
#+begin_src emacs-lisp
  (use-package org
    ...
    (setq org-structure-template-alist
          (append org-structure-template-alist
                  '(("sh" . "src shell")
                    ...
                    ("scpp" . "src c++")
                    ("sr" . "src rust"))))
    (setq org-directory (file-truename "~/Documents/Google Driver/All Notes")
          org-todo-keywords
    ...
    (org-ellipsis "▼")
    (org-log-done 'time)
    (org-agenda-files (list "~/Documents/Google Driver/All Notes/Agenda/work.org"
...
首先安装latex，这里使用的是 openSUSE-Tumbleweed

#+begin_src shell
sudo zypper refresh
sudo zypper install texlive                # 基础，~几百 MB
sudo zypper install texlive-latexextra     # 常用宏包集合（含 ctex、xecjk、fontspec）
sudo zypper install texlive-biblatex       # 文献
sudo zypper install texlive-ctex           # 中文核心，zypper 拆包后单独装更准
sudo zypper install latexmk ghostscript
#+end_src

安装字体

#+begin_src shell
sudo zypper install google-noto-sans-cjk-fonts google-noto-serif-cjk-fonts
# 或 Adobe 源： adobe-source-han-sans adobe-source-han-serif
#+end_src

安装emacs插件，auctex, auctex-latexmk, pdf-tools

=C-c C-c= 编完， =C-c C-v= 直接在Emacs里使用 pdf-tools 查看PDF

#+begin_src emacs-lisp
(use-package auctex
  :config
  (setq TeX-PDF-mode t)
  (setq TeX-engine 'xetex)           ; 中文用 xelatex
  (setq TeX-auto-save t)
  (setq TeX-parse-self t)
  (setq-default TeX-master nil))

;; 用 latexmk 做编译驱动，比裸 xelatex 省心（自动跑 bibtex/多次编译）
...
;; 预览器：WSLg 下用 pdf-tools（Emacs 内预览）最顺
;; 需要先 sudo zypper install poppler-glib 再 M-x package-install pdf-tools
